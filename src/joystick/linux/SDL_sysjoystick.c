    ...
    if ((ioctl(fd, EVIOCGBIT(EV_KEY, sizeof(keybit)), keybit) >= 0) &&
        ...
        for (i = BTN_JOYSTICK; i < KEY_MAX; ++i) {
            if (test_bit(i, keybit)) {
                ...
                if (joystick->nbuttons < SDL_IDX_MAX)
                    joystick->hwdata->idx_key_map[joystick->nbuttons] = i;
                ++joystick->nbuttons;
            }
        }
        ...
        for (i = 0; i < ABS_MAX; ++i) {
            ...
            if (test_bit(i, absbit)) {
                ...
                if (joystick->naxes < SDL_IDX_MAX)
                    joystick->hwdata->idx_abs_map[joystick->naxes] = i;
...
LINUX_JoystickOpen(SDL_Joystick * joystick, int device_index)
{
    ...
    // Need to distinguish assigned axes and buttons from unassigned
    // Note that 255 is reserved for AT keyboard driver,
    // and is explicitly excluded from being used as a valid key.
    SDL_memset(&joystick->hwdata->key_map, 255, sizeof(joystick->hwdata->key_map));
    SDL_memset(&joystick->hwdata->abs_map, 255, sizeof(joystick->hwdata->abs_map));
...
int SDL_JoystickGetAxisUsage(SDL_Joystick * joystick, int axis)
{
    if (axis < SDL_IDX_MAX)
    {
        axis = joystick->hwdata->idx_abs_map[axis];
        if (axis >= ABS_X && axis <= ABS_HAT3Y)
        return axis - ABS_X + 1;
    }
    return 0;
}

int SDL_JoystickGetButtonUsage(SDL_Joystick * joystick, int button)
{
    if (button < SDL_IDX_MAX)
    {
        button = joystick->hwdata->idx_key_map[button];
        if (button >= BTN_JOYSTICK && button <= BTN_THUMBR)
            return button - BTN_JOYSTICK + 1;
    }
    return 0;
}
